        ...
        <div class="card-body">
            ...
            <form class="needs-validation" novalidate action="form.php<?= $id ? "?id={$id}" : '' ?>" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="form-group mt-3">
                            <label for="turma_numero">Período</label>
                            <select class="form-control" id="turma_periodo" name="turma_periodo" required>
                                <option value=""></option>
                                <?php foreach (Utils::getPeriodo() as $key => $situacao) { ?>
                                    <option value="<?= $key ?>" <?= ($id and $key == $turma->turma_periodo) ? 'selected' : '' ?>><?= $situacao ?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
